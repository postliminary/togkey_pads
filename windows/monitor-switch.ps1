...
if ([Threading.Thread]::CurrentThread.GetApartmentState() -ne "STA") {
    throw "The tray app requires STA mode. Start it with powershell.exe -STA -ExecutionPolicy Bypass -File `"$PSCommandPath`"."
}
...
public sealed class MonitorHotkeyWindow : NativeWindow, IDisposable
{
    ...
    public MonitorHotkeyWindow()
    {
        ...
        if (!RegisterHotKey(Handle, 1, modifiers, VK_F23))
        {
            ...
            DestroyHandle();
            throw new Win32Exception(error, "Could not register Ctrl+Alt+F23");
        }
        ...
        if (!RegisterHotKey(Handle, 2, modifiers, VK_F24))
        {
            ...
            DestroyHandle();
            throw new Win32Exception(error, "Could not register Ctrl+Alt+F24");
        }
    }
...
function Show-PadPocketStatus {
    ...
    if ($null -ne $script:trayIcon) {
        $script:trayIcon.BalloonTipTitle = "Pad Pocket"
...
function Set-PadPocketInput {
    ...
    try {
        ...
        if ($WakeDelayMilliseconds -gt 0) {
            Start-Sleep -Milliseconds $WakeDelayMilliseconds
        }
        $changed = [NativeMonitorControl]::SetInput($Monitor, $Value)
        Show-PadPocketStatus "Switched $changed to $Name (VCP $Value)."
    }
    catch {
        Show-PadPocketStatus $_.Exception.Message ([System.Windows.Forms.ToolTipIcon]::Error)
    }
}
...
try {
    $available = [NativeMonitorControl]::DescribeMonitors()
    Write-Host "Monitors: $available"

    $hotkeyWindow = [MonitorHotkeyWindow]::new()
    $hotkeyWindow.add_HotkeyPressed({
        param($sender, $eventArgs)
        if ($eventArgs.Id -eq 1) {
            Set-PadPocketInput 2 "USB-C"
        }
        elseif ($eventArgs.Id -eq 2) {
            Set-PadPocketInput 3 "DisplayPort"
        }
    })
    ...
    if (-not $NoTray) {
        ...
        $usbItem = $menu.Items.Add("Switch to USB-C")
        $usbItem.add_Click({ Set-PadPocketInput 2 "USB-C" })
        $displayPortItem = $menu.Items.Add("Switch to DisplayPort")
        $displayPortItem.add_Click({ Set-PadPocketInput 3 "DisplayPort" })
        [void]$menu.Items.Add([System.Windows.Forms.ToolStripSeparator]::new())
        $statusItem = $menu.Items.Add("Show detected monitors")
        $statusItem.add_Click({ Show-PadPocketStatus ([NativeMonitorControl]::DescribeMonitors()) })
        [void]$menu.Items.Add([System.Windows.Forms.ToolStripSeparator]::new())
        ...
        $script:trayIcon.Text = "Pad Pocket monitor switch"
        $script:trayIcon.ContextMenuStrip = $menu
        $script:trayIcon.Visible = $true
        $script:trayIcon.add_DoubleClick({ Show-PadPocketStatus "Listening for Ctrl+Alt+F23/F24. Target: $Monitor" })
    }

    Show-PadPocketStatus "Ready. F23 selects USB-C; F24 selects DisplayPort. Target: $Monitor"
